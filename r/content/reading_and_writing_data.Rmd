...
my_table_Table$create <- read_parquet("my_table.parquet", as_data_frame = FALSE)
my_table_Table$create
```

```{r, read_parquet_table_class}
class(my_table_Table$create)
```
```{r, test_read_parquet_table_class, opts.label = "test"}
test_that("read_parquet_table_class works as expected", {
  expect_s3_class(my_table_Table$create, "Table")
})
```
