...
## Filter by values matching a predicate
...
## Filter by values matching a mask filter

You want to search for values in an Array via a mask filter.

### Solution

```{r, array_mask}
my_values <- Array$create(c(1:5, NA))
my_values[c(TRUE, FALSE, TRUE, FALSE, FALSE, TRUE)]
```
```{r, test_array_mask, opts.label = "test"}
test_that("array_mask works as expected", {
  expect_equal(my_values[c(TRUE, FALSE, TRUE, FALSE, FALSE, TRUE)], Array$create(c(1L, 3L, NA)))
})
```
