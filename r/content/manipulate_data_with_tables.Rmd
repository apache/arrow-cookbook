...
```{r}
# just keep `name` and `release_date` columns and drop the rest
popular_movies_arrow %>%
...
```{r}
# which movies are less than 2 hours long?
popular_movies_arrow %>%
...
```{r}
# add a column converting from the budget in millions to the actual number
popular_movies_arrow %>%
...
```{r}
# work out the mean run time depending on if a move is based on a novel or not
popular_movies_arrow %>%
...
```{r}
# append the source-dependent mean runtime to the original data
popular_movies_arrow %>%
  group_by(based_on_novel) %>%
