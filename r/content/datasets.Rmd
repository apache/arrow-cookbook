...
also work with multi-files imported as an Arrow Dataset.  This allows you to split data across 
multiple files/folders using partitioning, avoiding problems 
...
read in the necessary files needed for any given analysis.

It's possible to have partitioned data in Parquet, Feather (aka Arrow), and CSV (or 
other text-delimited) formats.  If you are choosing a partitioned or multi-file format, we 
...
You want to write an Arrow Dataset to disk in a single Parquet file.
...
write_dataset(dataset = starwars |> select(!where(is.list)), path = "starwars_data_csv", format = "csv")
```
